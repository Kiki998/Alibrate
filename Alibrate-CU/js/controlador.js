...
(()=>{
    ...
    let textosDePrueba=[
        "Este es un libro que te puede interesar, se encuentra en muy buen estado",
        "Costo del libro:400lps"
    ]
    
    let textosDePrueba2=[
    
        "Este es un libro que te puede servir mucho para tus clases de Ingeniería. El libro se encuentra en muy buen estado y a un buen precio",
        "Costo del libro: 400lps"
        ]
    ...
    let textosDePrueba3=[
            ...
            "Este libro es muy malo, no me gusto",
            "El autor es de mis favoritos, este libro es increible",
            "Es muy malo",
            "X2"
    ]
    ...
    let contador = 1;
    for (let i=1;i<6;i++){//Generar 5 categorias
        let categoria = {
        ...
        for (let j=0;j<10;j++){//Generar 10 apps por categoria
            let aplicacion = {
                codigo:contador,
                nombre:"Book "+contador,
                descripcion:textosDePrueba,
                icono:`img/libros-cu/${contador}.webp`,
                instalada:contador%3==0?true:false,
                app:"app/demo.apk",
                calificacion:Math.floor(Math.random() * (5 - 1)) + 1,
                descargas:1000,
                desarrollador:`Desarrollador ${(i+1)*(j+1)}`,
                imagenes:["img/Alibrate/2.webp","img/Alibrate/3.webp"],
                comentarios:[
                    {comentario:textosDePrueba3[Math.floor(Math.random() * (5 - 1))],calificacion:Math.floor(Math.random() * (5 - 1)) + 1,fecha:"12/12/2012",usuario:"David Flores"},
                    {comentario:textosDePrueba3[Math.floor(Math.random() * (5 - 1))],calificacion:Math.floor(Math.random() * (5 - 1)) + 1,fecha:"12/12/2012",usuario:"Angie Murcia"},
                    {comentario:textosDePrueba3[Math.floor(Math.random() * (5 - 1))],calificacion:Math.floor(Math.random() * (5 - 1)) + 1,fecha:"12/12/2012",usuario:"Jossue Ochoa"},
                ]
            };
            contador++;
...
function generarModal(index){
    ...
    const boton = !app.instalada ? '<button type="button" class="btn btn-success">Comprar</button>' : '';
